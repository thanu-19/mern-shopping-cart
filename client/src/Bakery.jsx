...
const Bakery = () => {
  ...
  const fetchItems = async () => {
    const res = await axios.get(`${backendURL}/bakery`);
    setItems(res.data);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await axios.put(`${backendURL}/bakery/${editingItem._id}`, data, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        setEditingItem(null);
      } else {
        await axios.post(`${backendURL}/bakery`, data, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${backendURL}/bakery/${id}`);
      fetchItems();
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="main-content">
        <h1 className="title">🍰 Bakery Management</h1>
        ...
        <h2 className="sub-title">📋 All Bakery Items</h2>
